...
It could happen that clippy complains about rustlings stuff in our repo, so we narrow it down to the package `badlib` to reduce noise.
In a realistic project, you'd omit the `--package` flag to ensure your entire workspace is linted.
Add the following CI step to the existing ones.

```yml
- name: Check code quality with clippy
  run: cargo clippy --package badlib -- --deny warnings
```
...
The `Cargo.toml` of your shippable binary must include the URL to your GitHub repository:

```toml
# in day_5/shippable/Cargo.toml
edition = "2021"
...
There is a small annoyance with rustlings being in the same repository.
If any of the rustlings exercises aren't completed yet (which is probably the case), the build will fail.
For now, it's easiest to exclude rustlings from our workspace.
Open `Cargo.toml` at the root of your repository and remove the `"rustlings"` element in the `members` list:

```toml
members = ["day_?/*", "rustlings"]
#                   ^^^^^^^^^^^^^
#                   delete this
```
